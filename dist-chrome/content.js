...
function showInputBox(selectionText) {
  dismissInputBox();
...
function handleDocumentClickForInputBox(e) {
  if (currentInputBox && !currentInputBox.shadowRoot?.contains(e.target)) {
    dismissInputBox();
  }
}
function handleEscapeForInputBox(e) {
  if (e.key === "Escape") {
    dismissInputBox();
  }
}
...
function createToast(options, position) {
  dismissToast();
  const { message, type, duration = 2e4 } = options;
  const container = document.createElement("div");
...
function handleEscapeForToast(e) {
  if (e.key === "Escape") {
    dismissToast();
  }
}
